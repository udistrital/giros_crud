...
type Giro struct {
	...
	CuentaBancariaId  *CuentaBancaria `orm:"column(cuenta_bancaria_id);rel(fk)"`
	FormaPagoId       *FormaPago      `orm:"column(forma_pago_id);rel(fk)"`
	Vigencia          int             `orm:"column(vigencia)"`
	AreaFuncional     int             `orm:"column(area_funcional)"`
	TipoDocumentoId   int             `orm:"column(tipo_documento_id)"`
	TipoGiroId        int             `orm:"column(tipo_giro_id)"`
	NumeroSeleccion   int             `orm:"column(numero_seleccion)"`
